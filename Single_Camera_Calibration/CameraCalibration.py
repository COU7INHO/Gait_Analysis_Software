...
class CalibrateCamera:
    ...
    def findChessboardCorners(
        self, squareSize=25, chessboardRows=9, chessboardCols=6, imshow=False
    ):
        ...
        for index in range(len(self.indices)):
            ...
            for img in images:
                ...
                self.ret, corners = cv.findChessboardCorners(
                    self.gray, self.chessboardSize, None
                )
                ...
                if self.ret == True:
                    ...
                    if self.imshow == True:
                        cv.drawChessboardCorners(
                            self.frame, self.chessboardSize, corners2, self.ret
                        )
        ...
        for index in range(len(self.indices)):
            ...
            for img in images:
                ...
                if self.ret == True:
                    ...
                    self.ret, mtx, dist, rvecs, tvecs = cv.calibrateCamera(
                        self.objpoints,
                        self.imgpoints,
                        self.gray.shape[::-1],
                        None,
                    ...
                    cv_file = cv.FileStorage(
                        f"./Single_Camera_Calibration/map{index}.xml",
                    ...
                    for i in range(len(self.objpoints)):
                        ...
                        error = cv.norm(
                            self.imgpoints[i], imgpoints2, cv.NORM_L2
                        ) / len(imgpoints2)
                        mean_error += error
