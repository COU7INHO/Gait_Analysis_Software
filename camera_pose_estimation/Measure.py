...
class Measure(CalibratedImages):
    ...
    def calibrated_env(self):
        while self.cam1.isOpened() and self.cam2.isOpened():
            corners1, _, _ = cv2.aruco.detectMarkers(
                self.img1, ARUCO_DICT, parameters=PARAMETERS
            )
            corners2, _, _ = cv2.aruco.detectMarkers(
                self.img2, ARUCO_DICT, parameters=PARAMETERS
            )
            ...
            if corners2:
                ...
                for cnt2 in contours2:
                    ...
                    cv2.putText(
                        self.img2,
                        "Width {} cm".format(round(object_width2, 1)),
                        (int(x2 - 100), int(y2 - 20)),
                        cv2.FONT_HERSHEY_PLAIN,
                        2,
                        (50, 255, 0),
                        2,
                    ...
                    cv2.putText(
                        self.img2,
                        "Height {} cm".format(round(object_height2, 1)),
                        (int(x2 - 100), int(y2 + 15)),
                        cv2.FONT_HERSHEY_PLAIN,
                        2,
                        (0, 255, 255),
                        2,
