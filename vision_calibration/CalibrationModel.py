...
class CalibrationModel(GetCalibrationImages):
    ...
    def findChessboardCorners(self, imshow=False):
        ...
        objp[:, :2] = np.mgrid[
            0 : self.chessboardrows, 0 : self.chessboardcols
        ].T.reshape(-1, 2)
        ...
        for imgLeft, imgRight in zip(imagesLeft, imagesRight):
            ...
            self.retL, cornersL = cv2.findChessboardCorners(
                self.grayL, self.chessboardsize, None
            )
            self.retR, cornersR = cv2.findChessboardCorners(
                self.grayR, self.chessboardsize, None
            )
            ...
            if self.retL and self.retR == True:
                ...
                cornersL = cv2.cornerSubPix(
                    self.grayL, cornersL, (11, 11), (-1, -1), CRITERIA
                )
                self.imgpointsL.append(cornersL)

                cornersR = cv2.cornerSubPix(
                    self.grayR, cornersR, (11, 11), (-1, -1), CRITERIA
                )
                self.imgpointsR.append(cornersR)

                cv2.drawChessboardCorners(
                    self.imgL, self.chessboardsize, cornersL, self.retL
                )
                cv2.drawChessboardCorners(
                    self.imgR, self.chessboardsize, cornersR, self.retR
                )
    ...
    def calibration(self):
        (
            ...
            self.objpoints, self.imgpointsL, self.framesize, None, None
        )
        heightL, widthL, self.channelsL = self.imgL.shape
        self.newCameraMatrixL, self.roi_L = cv2.getOptimalNewCameraMatrix(
            cameraMatrixL, self.distL, (widthL, heightL), 1, (widthL, heightL)
        )
        ...
        (
            ...
            self.objpoints, self.imgpointsR, self.framesize, None, None
        )
        heightR, widthR, self.channelsR = self.imgR.shape
        self.newCameraMatrixR, self.roi_R = cv2.getOptimalNewCameraMatrix(
            cameraMatrixR, self.distR, (widthR, heightR), 1, (widthR, heightR)
        )

    def stereoCalculation(self):
        (
            self.retStereo,
            self.newCameraMatrixL,
            self.distL,
            self.newCameraMatrixR,
            self.distR,
            self.rot,
            self.trans,
    ...
    def newMatrix(self):
        ...
        cv_file = cv2.FileStorage(
            "./vision_calibration/stereoMap.xml", cv2.FILE_STORAGE_WRITE
        )
