        ...
        if file_path:
            ...
            for paragraph in paragraphs:
                if c.stringWidth(paragraph) > right_margin - self.X_MARGIN:
                    ...
                    line = ''
                    for word in words:
                        if c.stringWidth(line + ' ' + word) < right_margin - self.X_MARGIN:
                            line += ' ' + word
                        else:
                            c.drawString(self.X_MARGIN, y, line)  # Draw the line
                            y -= 20  # Move to the next line
                            line = word
                    c.drawString(self.X_MARGIN, y, line)  # Draw the last line of the paragraph
                    y -= 20  # Move to the next line
                else:
                    c.drawString(self.X_MARGIN, y, paragraph)  # Draw the entire paragraph
                    y -= 20  # Move to the next line
